...
namespace WPSExcelDna
{
    public static class WpsReg
    {
        /// <summary>
        /// 判断是否为Wps
        /// </summary>
        static bool iswps
        {
            get
            {
                ...
                FileVersionInfo fileinfo = FileVersionInfo.GetVersionInfo(excel.FileName);
                return fileinfo.OriginalFilename.ToLower() == "et.exe";
            }
        }
        ...
        /// 对wps进行注册
        /// </summary>
        /// <param name="num">默认开启Ribbon和CTP</param>
        public static void wpsRegistry(int num = 2)
        {
            if (iswps)
            {
                // 获取wps注册表中位置
                RegistryKey key = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\Kingsoft\Office\ET\AddinsWL\", true);
                // 根据EXCELDNA的算法生成ribbon和ctp的progid并写入注册表，两者同一套算法
                for (int i = 0; i < num; i++)
                {
                    var progid = GetProgid.getProgId(ExcelDnaUtil.XllPath, i);
                    var _progid = progid.Replace('.', '_');
                    if (key.GetValue(_progid) == null || key.GetValue(progid) == null)
                    {
                        ...
                        if (i == num - 1)
                        {
                            //Console.WriteLine("提示：即将关闭wps，完成相关注册，再次打开WPS插件将自动加载！");
                            //MessageBox.Show("提示：即将关闭wps，完成相关注册，再次打开WPS插件将自动加载！");
                            ((Excel.Application)ExcelDnaUtil.Application).Application.Quit();
                        }
                    }
                }
                key.Close();
        ...
        public static void wpsUnRegistry(int num = 2)
        {
            if (iswps)
            {
                RegistryKey key = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\Kingsoft\Office\ET\AddinsWL\", true);
                ...
                key.Close();
                //Console.WriteLine("提示：即将关闭wps，完成插件卸载,及注册表清理！");
                //MessageBox.Show("提示：即将关闭wps，完成插件卸载,及注册表清理！");
                ((Excel.Application)ExcelDnaUtil.Application).Application.Quit();
            }
        }
